...
function Make_Query_For_Local_Council($connect,$campus,$college,$program,$year,$position)
{
  if($campus == "Tagum"){
    if($position == "Senator" || $position == "Bussines Manager"){
      ...
      }elseif($program == "BS Information Technology"){
    ...
    $lcname = $program;
  }elseif($campus == "Obrero"){
    $query = 'SELECT * FROM tbnominees WHERE position = "'.$position.'" AND campus = "'.$campus.'" AND college ="'.$college.'"  AND indicator = "local council" ORDER BY id ASC ';
    $result = mysqli_query($connect, $query);
    $lcname = $college;
  }
  else{
